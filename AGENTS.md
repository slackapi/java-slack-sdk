...
- **Snapshot**: Automatically published on every push to `main`. Can also be triggered manually via `workflow_dispatch` on any branch with a `-SNAPSHOT` version.
- **Stable**: Run `scripts/set_version.sh 1.x.y`, commit, tag `v1.x.y`, push to `main` — the [release workflow](.github/workflows/release.yml) publishes to Maven Central automatically. Can be re-triggered via `workflow_dispatch` if needed.
