...
public class IgnoringSelfEvents implements Middleware {
    ...
    private final boolean ignoringSelfAssistantMessageEventsEnabled;

    public IgnoringSelfEvents(SlackConfig config) {
        this(config, false);
    }

    public IgnoringSelfEvents(SlackConfig config, boolean ignoringSelfAssistantMessageEventsEnabled) {
        this.gson = GsonFactory.createSnakeCase(config);
        this.ignoringSelfAssistantMessageEventsEnabled = ignoringSelfAssistantMessageEventsEnabled;
    }
        ...
        if (req.getRequestType() == RequestType.Event) {
            ...
            if (eventElem != null) {
                ...
                if (eventBotUserId != null && eventBotUserId.equals(appBotUserId)) {
                    if (!this.ignoringSelfAssistantMessageEventsEnabled && EventsApiPayloadParser.isMessageEventInAssistantThread(eventElem)) {
                        // Let Assistant#botMessage handle this pattern
                        return chain.next(req);
                    }
